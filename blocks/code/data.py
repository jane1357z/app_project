...
class Code_data:
    ...
    def get_number_of_questions(self, table_name):
        df = self.get_dataframe(table_name)
        return len(df)
    ...
    def insert_session(self, code, status):
        table_name = "Example_Session"
        col = "Code"
        row_check = self.get_row(table_name, code, col)
        if row_check == False:
            row_data = (code, status)
            columns = 'Code, Session_status'
            self.insert_row(columns, row_data, table_name)
        else:
            query = f"UPDATE {table_name} SET Session_status = '{status}' WHERE {col} = '{code}';"
            try:
                conn = self.init_connection()
                cursor = conn.cursor()
    ...
    def update_code_status(self, code, status, table_name):
        query = f"UPDATE {table_name} SET Status = '{status}' WHERE Code = '{code}';"
        try:
            conn = self.init_connection()
            cursor = conn.cursor()
            cursor.execute(query)
